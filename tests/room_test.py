...
class TestRoom(unittest.TestCase):
    ...
    def test_add_song(self):
        song_list = self.instance_of_room.add_song(self.instance_of_song.name_of_song)
        self.assertEqual(["Unholy"], song_list)
    
    def test_add_guest_to_room(self):
        guest = self.instance_of_room.check_in_guest(self.instance_of_guest_1.name)
        self.assertEqual(["Bob"], guest)
    
    def test_checkout_guest_from_room(self):
        guest_check_out = self.instance_of_room.check_out_guest()
        self.assertEqual([], guest_check_out)

    def test_guest_in_room_is_true(self):
        self.instance_of_room.check_in_guest(self.instance_of_guest_1.name)
        is_guest_in_room = self.instance_of_room.check_in_guest(self.instance_of_guest_2.name)
        self.assertEqual(False, is_guest_in_room)

    def test_guest_in_room_is_false(self):
        is_guest_in_room = self.instance_of_room.check_in_guest(self.instance_of_guest_1.name)
        self.assertEqual(["Bob"], is_guest_in_room)
    
    # def show_person_in_room(self):
    #     self.instance_of_room.check_in_guest(self.instance_of_guest_1.name)
    #     self.instance_of_room_2.check_in_guest(self.instance_of_guest_2.name)
    #     room_info = self.instance_of_room.room_info()
